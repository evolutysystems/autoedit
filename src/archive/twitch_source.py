...
# 解決順: 絶対パス → 同梱物(exe 隣接) → PATH(shutil.which) → 開発時 `python -m twitchdl` → 設定値。
def _resolve_twitchdl(configured):
    ...
    if os.path.isabs(configured) and os.path.isfile(configured):
        return [configured]
    # 2) 同梱物 (twitch-dl.exe を exe 隣接/_internal 基準で探す)
    ...
    if found:
        return [found]
    # 4) 開発実行時のフォールバック: モジュールが import 可能なら python -m twitchdl
    if not getattr(sys, "frozen", False):
        try:
            ...
            pass
    # 5) 見つからず: 設定値をそのまま (実行時にエラー通知)
    return [configured]


# サブプロセスを実行し、標準出力/エラーを progress_cb に流す。非0終了で TwitchError。
def _run(argv, progress_cb=None, label="取得中…"):
...
def download_vod(video_id, work_dir, twitch_dl_path="twitch-dl", quality="source",
                 auth_token="", start=None, end=None, progress_cb=None):
    ...
    _run(argv, progress_cb, label="VOD取得中…")
...
def download_chat(video_id, work_dir, twitch_dl_path="twitch-dl", progress_cb=None):
    ...
    _run(argv, progress_cb, label="コメント取得中…")
    if not os.path.isfile(expected):
