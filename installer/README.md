# Stretheus インストーラー (ビルド & リリース手順)

`docs/request/resolve8.md` に基づくダウンローダ型インストーラー `StretheusSetup.exe` のビルドと、
...
| PyInstaller | 本体 `dist/Stretheus/` の生成 | `docs/HowToRelease.md §5`（既存） |
...
```powershell
# リポジトリ直下で実行
Compress-Archive -Path "src/dist/Stretheus/*" -DestinationPath "$env:TEMP/Stretheus-v1.0.0.zip"
$hash = (Get-FileHash "$env:TEMP/Stretheus-v1.0.0.zip" -Algorithm SHA256).Hash
...
```powershell
# 例) 7-Zip で 1900MB 単位に分割 (= Stretheus-v1.0.0.zip.001, .002, ...)
& "C:\Program Files\7-Zip\7z.exe" a -tzip -v1900m "$env:TEMP/Stretheus-v1.0.0.zip" ...
...
| `PayloadParts` | 単一: `Stretheus-v1.0.0.zip`<br>分割: `Stretheus-v1.0.0.zip.001,Stretheus-v1.0.0.zip.002` | ダウンロードする payload のファイル名（カンマ区切り） |
...
> **【重要・自動更新】** `MyAppVersion` は **`src/version.py` の `__version__` と必ず一致させる**
> （`docs/request/request_autoupdate.md §4`）。アプリは起動時に GitHub Releases の最新タグと
> `__version__` を比較して更新を提案するため、両者がずれると更新判定が誤動作する。
> リリース毎に **`.iss` の `MyAppVersion` と `src/version.py` の両方**を更新すること。
>
> **【更新時の setting.json 保持】** 本インストーラは、既存インストール（`{app}\_internal\src\settings\setting.json` が存在）
> を検知した場合、**展開前に既存 setting.json を退避 → 展開後にそのまま復元**し、ユーザーが設定した既存値を
> 一切上書きしない（出力先選択ページもスキップして既存 `output_directory` を維持）。新バージョンで追加された
> 新規キーはアプリ起動時に既定補完される（`request_autoupdate.md §8`）。
...
# → installer\Output\StretheusSetup.exe が生成される
```
...
gh release create v1.0.0 `
  "installer\Output\StretheusSetup.exe" `
  "$env:TEMP\Stretheus-v1.0.0.zip" `
  --repo evolutysystems/homepage `
  --title "Stretheus v1.0.0" `
  --notes "インストーラー配布 (CPU版)"

# 分割 payload の場合は part を列挙して添付
gh release upload v1.0.0 `
  "$env:TEMP\Stretheus-v1.0.0.zip.001" "$env:TEMP\Stretheus-v1.0.0.zip.002" `
  --repo evolutysystems/homepage
```
...
// SYSTEMS[].downloadUrl
downloadUrl:
  'https://github.com/evolutysystems/homepage/releases/download/v1.0.0/StretheusSetup.exe',
```
...
- [ ] `StretheusSetup.exe` 実行 → 言語/ようこそ → インストール先選択
...
- [ ] スタートメニュー／デスクトップのショートカットから `Stretheus.exe` が起動する
...
| インストール先既定 | `{localappdata}\Programs\Stretheus`（昇格不要） | §9-5 |
| 出力先既定 | `{userdocs}\Stretheus\output` | §9-7 |
| UI 言語 | 日本語 | §9-9 |
