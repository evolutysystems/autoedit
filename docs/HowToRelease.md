...
| 区分 | 内容 | 理由・注意 |
|---|---|---|
| 実行ファイル | `AutoEdit.exe`（PyInstaller 生成） | GUI ランチャ（`main_window.py`）を起点に起動 |
...
`ffmpeg_runner.get_ffmpeg_exe()` は既定で `setting.json` の `ffmpeg.executable`（既定値 `"ffmpeg"`）を返し、
PATH 解決に依存する（`ffprobe` も同様）。**ダウンロード版ユーザーの PATH に FFmpeg がある保証は無い**ため、
次のいずれかで PATH 非依存にする。

- **推奨**: `ffmpeg.exe` / `ffprobe.exe` を配布物へ同梱し、同梱 `setting.json` の値を相対/絶対パスで指す。
  ```json
  "ffmpeg": {
  ...
  ```
- 代替: README で「FFmpeg を別途インストールし PATH を通す」ことを必須要件として明記する（同梱より体験は劣る）。

> 配布物のレイアウト例（§5.3）では exe と同じ階層に `ffmpeg/` フォルダを置く前提。
...
（例: `%APPDATA%\AutoEdit\`）へ寄せる改修が望ましい（別タスク）。ログも `logger.get_logger(log_dir="logs")`
...
- `icon='gui/app.ico'` は**アイコン未配置のため省略**。用意できたら `EXE(...)` に追加する（任意）。
...
成功すると `src/dist/AutoEdit/` に `AutoEdit.exe` と依存一式が生成される
...
```
AutoEdit/                      ← これを zip 化して配布
├─ AutoEdit.exe                ← メイン画面(main_window.py)起点の実行ファイル
...
- [ ] `AutoEdit.exe` をダブルクリックでメイン画面（`MainWindow`）が表示される。
- [ ] 「参照...」で入力動画を選択できる。
...
- **Authenticode 証明書**（OV/EV）で `AutoEdit.exe` と主要 dll に署名すると SmartScreen 評価が改善する
  （EV はより強力）。
  ```powershell
  signtool sign /fd SHA256 /tr http://timestamp.example /td SHA256 /a "dist/AutoEdit/AutoEdit.exe"
  ```
- 署名は外部公開を伴う作業のため、証明書の管理・運用ポリシーに従って実施する。

### 7.3 配布パッケージとチェックサム

- 配布は **インストーラ方式**（`AutoEditSetup.exe`）を基本とする（`docs/request/resolve8.md`）。
  ユーザーは小さいインストーラーをDL→実行し、インストーラーが本体ペイロードを GitHub Releases から取得する。
  zip 直配布は後方互換の代替として残す。
- 改ざん検知用に **SHA-256** を公開する。
  ```powershell
  Compress-Archive -Path "dist/AutoEdit/*" -DestinationPath "AutoEdit-v1.0.0.zip"
  Get-FileHash "AutoEdit-v1.0.0.zip" -Algorithm SHA256   # ← インストーラの PayloadSHA256 に設定
  ```
...
- 配布ファイル名例: `AutoEdit-v1.0.0-win64.zip`。
