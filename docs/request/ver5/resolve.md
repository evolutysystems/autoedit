...
**§6 の Phase 1〜8 をすべて実装した。**確認事項 (§9) はすべて §9 の決定どおり。
テストは 933 件すべて通る (ポイント・透かし関連は 100 件)。

| Phase | 内容 | 結果 |
| --- | --- | --- |
| 1 | `points.py` (予約 / 確定 / 解放・サブスクキャッシュ・commit キュー) | 完了 |
| 2 | `watermark_overlay.py` (チェーン生成・単独パス) | 完了 |
| 3 | 経路 A / B への組み込み (`pipeline_runner` / `output_writer` / `PipelineContext`) | 完了 |
| 4 | 経路 C / D への組み込み (`clip_writer`) | 完了 |
| 5 | `renderer._composite` への混ぜ込み | 完了 |
| 6 | Resolve 書き出しへの組み込み (`resolve_export` / `fcpxml_builder`) | 完了 |
| 7 | UI (残高インジケータ・確認ダイアログ・アカウントタブ) | 完了 |
| 8 | 設定・ドキュメント更新 | 完了 |
| — | **実素材での見え方の確認** (透かしの視認性・字幕との重なり) | **未実施** (実素材が要る) |

### 実装で本書の案から変えたこと

| # | 項目 | 本書の案 | 実装 | 理由 |
| --- | --- | --- | --- | --- |
| 1 | 予約する入口 | §3.2 は `run_pipeline` / `write_clips` / `export_*` の 3 系統 | **再開経路も予約する** (`run_from_project` / `run_from_archive_project`) | 「編集の続き」から書き出せば出力 1 本ができる。予約しないと、そこだけ無償で出せてしまう |
| 2 | アーカイブの透かしの入れ方 | §5.4 は「個別出力・結合出力のそれぞれについて確認する」 | **各クリップの本編へ入れる** (経路 C は合成へ混ぜ込み、経路 D は焼き込み後に 1 パス) | 本編に入れたうえで結合後にもう 1 パス足すと、本編部分だけ二重に焼き込まれて濃さが変わる。個別出力・結合出力のどちらにも入るため確認事項 #6 は満たす。**イントロカードと OP/ED には入らない** |
| 3 | `export_timeline` の `jobType` | 確認事項 #3 は「Timeline へ `origin` を新設してプロジェクト JSON へ保存する」 | **`source.archive` の有無で判定する** (`project_io.project_kind`) | アーカイブ用 Timeline は既に `source.archive` を持ち、保存も復元もされている。新しい項目と旧プロジェクトの移行を増やさずに同じ判定ができる |
| 4 | 確認ダイアログの呼び出し口 | §5.6 は「出力ボタン押下時」 | **2 つ用意した。** 動画出力はワーカースレッドから `WatermarkConfirmBridge`、Resolve 書き出しは GUI スレッドから `watermark_confirm()` | Resolve 書き出しは GUI スレッドで走る。そこでブリッジ (Event 待ち) を使うとイベントループが止まり固まる |
| 5 | サブスク導線 | §5.6 R13 は「Twitch のサブスクページを開く」 | **`points.subscribe_url` 設定にした。空のあいだはボタンを出さない** | サブスクページの URL は配信者ごとに違い、API も返さない。誤った URL を焼き込まないため設定にした |
| 6 | `PointsService` の持ち方 | §5.2 は「アプリ全体で 1 つ持ち回る」 | **`services/config.shared_points()` でプロセス共有にした** | 画面・出力処理・残高表示が別の実体を持つとリフレッシュの直列化 (§6.4) が効かない。CLI / テストは `current_points()` が None を返し、従来どおりポイントを通らない |
| 7 | FCPXML への透かし | §5.5 は「最上位トラックへ全長のクリップを足す」 | `spec["overlays"]` を新設し、`fcpxml_builder` が **asset / format / `<video lane="2">`** を書く。位置はタイトルと同じ近似規約 (`_to_export_space`) | 既存の builder は素材 1 本 (元動画) しか扱えなかった。位置の座標系はタイトルと 1 か所で共有する |

**未ログインのユーザーの体験は 1 バイトも変わらない。** 予約も透かしも通らず、
FFmpeg へ渡すコマンドは従来と同一である (テストで固定した)。
