...
- `src/settings/setting.json` の `subtitle` セクションに新規キーを追加する（要望①）。
- `src/modules/subtitle_generator.py` の字幕生成処理が新規キーを参照するよう修正する（要望①）。
- `src/settings/settings_window.py` の設定画面に**タブ**を追加し、「一般」「字幕」に分割する（要望①）。
- 設定画面に**「開始」ボタン**を設置し、押下で `src/main.py`（パイプライン）を実行する（要望②）。
...
設定画面に「開始」ボタンを設置し、押下で `src/main.py`（パイプライン）を実行する。

#### 配置

- 「保存」ボタンと並べて**タブ外（ウィンドウ下部）に共通配置**する（`_build_ui` の `QHBoxLayout`）。
  全タブに対する共通操作のため、字幕/一般いずれのタブでも押下できる。
- レイアウトは `[stretch] [保存] [開始]` とし、右寄せで並べる。

#### 実行スクリプトのパス解決

- モジュール定数 `MAIN_SCRIPT` を定義する。`SETTINGS_DIR`（`src/settings`）の親ディレクトリ（`src`）直下の
  `main.py` を指す：`os.path.join(os.path.dirname(SETTINGS_DIR), "main.py")`。
  ハードコードを避け `__file__` 基準で解決する（CLAUDE.md 準拠）。

#### 押下処理 `_on_start`

1. **最新 UI 値を保存**：まず `_save_current(show_message=False)` で現在の設定を保存する。
   保存に失敗した場合は処理を**中断**する（古い設定でパイプラインが走るのを防ぐ）。
2. **起動スクリプトの存在確認**：`MAIN_SCRIPT` が存在しなければ `QMessageBox.critical` で通知して中断する。
3. **別プロセスで起動**：GUI をブロックしないよう `subprocess.Popen([sys.executable, MAIN_SCRIPT], ...)` で
   非同期に実行する。
   - `cwd` は `MAIN_SCRIPT` のディレクトリ（`src`）に設定し、相対パス解決・ログ出力先を安定させる。
   - Windows では進捗（`run_ffmpeg_progress` のコンソール出力）を確認できるよう
     `creationflags=subprocess.CREATE_NEW_CONSOLE`（未定義環境では `0`）で新規コンソールを開く。
4. **結果通知**：起動成功時は「実行開始」を情報表示、`OSError` 時は「起動エラー」を表示する。

#### エラー処理方針

- 保存失敗・スクリプト不在・起動失敗はいずれも `QMessageBox` でユーザーに通知し、例外を伝播させない。
- 新規例外クラスは追加しない（既存の例外方針を踏襲）。

> **既知の制約（凍結版）**: PyInstaller で凍結した配布版には `src/main.py` が存在しないため、
> 「開始」ボタンは機能しない（`docs/HowToRelease.md §3.3` 注記）。凍結版のメイン処理は
> `gui/main_window.py` のワーカースレッド実行で完結するため通常利用に影響はないが、
> 設定画面単独起動時の当該ボタンは開発（非凍結）環境向けである点に留意する。
...
| `src/settings/settings_window.py` | `QTabWidget` 化、字幕タブに入力追加、`DEFAULT_SETTINGS`・`_load_to_ui`・`_on_save` 更新、**「開始」ボタン＋`_on_start`／`MAIN_SCRIPT` 追加（要望②）** |
...
2. `settings_window.py`: `DEFAULT_SETTINGS` 更新 → `QTabWidget` 化 → 字幕タブ UI 追加 → `_load_to_ui` / `_on_save` 更新 → **「開始」ボタン設置・`MAIN_SCRIPT` 定義・`_on_start` 実装（要望②）**。
...
5. 「開始」ボタン押下で設定が保存され、`main.py` が別プロセス（別コンソール）で起動することを確認（要望②）。
